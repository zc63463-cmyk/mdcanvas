...
> **判据落点说明**：F1 原始矩阵为「同语义」对比（对键错误天然不敏感——键不失效也只是重算同语义结果）；该对照的红点实际落在「换 measure + 换 key」等价用例与通道键用例——恰为计划该对照条目的意图：证明「键的判据」是缓存正确性的有效闸门（键不反映度量语义 → 陈旧几何必被逐位判据逮住）。
```
...
**commit `74303aa`** — `perf+docs: 森林布局缓存收口（基准脚本入库 + 前后对照 + CHANGELOG）`
